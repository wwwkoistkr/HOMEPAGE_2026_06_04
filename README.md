...
> ## 🆕 v42.3 변경 사항 (2026-06-03)
>
> **③ 원스톱 카드 +3cm 우측 폭 확장**: 히어로 원스톱(시뮬레이터) 카드를 **오른쪽으로 약 3cm(≈113px)** 넓힘. (요청 ①②④는 ③ 결과 확인 후 결정 예정 → 본 버전 미반영)
>
> 1. **그리드 비율 조정**: `.unified-hero-grid` `grid-template-columns: 62fr 38fr → 71fr 29fr`. cm 고정값 대신 **fr 비율 조정**으로 반응형 유지(데스크탑↔모바일 연동).
> 2. **결과**: 원스톱 카드 폭 **785px → 899px(+114px≈+3cm)**, 텍스트 카드 폭 481px → 367px로 축소. 전체 히어로 우측 끝선은 **1371px 그대로 유지**.
> 3. **캐시 버전** `v42.2 → v42.3`.
>
> **검증(데스크탑 1440 + 모바일 390)**: 원스톱 좌=69px / 우=**968px** / 폭=899px(+3cm 확장 확인). 텍스트 우측 끝=**1371px** = 고객지원 우측 끝 1371px → **차이 0px**. 두 카드 밑단 621px 일치, 겹침 없음(원스톱 우 968px < 텍스트 좌 1004px, gap 36px). 모바일 1열 스택(텍스트 위→원스톱 아래, 동일폭 326px) 정상. **8K clamp 상한 유지 + cm 고정값 없는 반응형(fr/clamp/vw)** 유지.
>
> ---
