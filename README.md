# KOIST Website v42.4.4

> ## 🆕 v42.4.4 변경 사항 (2026-06-04) — KOIST 평가 프로세스 바 글자색 진한 검정으로 통일
>
> **사용자 지시**: KOIST 평가 프로세스 바 안 글자(준비/평가 N개월)를 진한 검정색으로 변경. 배포·커밋·백업 완료.
>
> **변경 내역:**
> - **KOIST 평가 프로세스 바 안 글자(준비/평가 N개월)** → 금색 `#FBBF24` → 진한 검정 `#0F172A`, text-shadow 흰색 계열(rgba(255,255,255,0.85))로 주황·파랑 배경 위 가독성 확보. (CCRA 바와 동일 스타일로 통일)
> - **캐시 `v42.4.3→v42.4.4`**.
> - **프로덕션(koist.ai.kr) 배포 완료**.
